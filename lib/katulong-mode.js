...
export async function katulongMode(env, args) {
  ...
  const wsUrl = env.url.replace(/^http/, "ws") + "/ws/claude";
  ...
  // Register this claude session with katulong
  wsSend(ws, {
    type: "claude:start",
    session: env.session,
    prompt: prompt || null,
  ...
  // Handle Ctrl+C
  process.on("SIGINT", () => {
    wsSend(ws, { type: "claude:end", session: env.session, result: "aborted" });
    cleanup(ws);
  });
...
async function startAgent(ws, env, prompt) {
  ...
  try {
    ...
    wsSend(ws, { type: "claude:end", session: env.session, result: "error", error: "Agent SDK not available" });
    cleanup(ws);
    return;
  }
  ...
  try {
    ...
    wsSend(ws, { type: "claude:end", session: env.session, result: "error", error: err.message });
    cleanup(ws);
  }
}
...
async function runAgentLoop(sdk, ws, env, prompt) {
  if (!prompt) {
    ...
    wsSend(ws, { type: "claude:waiting-for-input", session: env.session });
  ...
  for await (const message of query({
    ...
    if (message.type === "assistant") {
      ...
      console.log("\nDone.");
      wsSend(ws, { type: "claude:turn-complete", session: env.session });
    }
  }
